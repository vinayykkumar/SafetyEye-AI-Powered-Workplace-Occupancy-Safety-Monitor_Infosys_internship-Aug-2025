...
# --- Safe initialization for Streamlit session variables ---
if "auto_email_enabled" not in st.session_state:
    st.session_state.auto_email_enabled = False
if "auto_email_thread" not in st.session_state:
    st.session_state.auto_email_thread = None

auto_email_flag = False
# ---------------- CONFIG ----------------
...
# ---------------- MODEL & PATHS ----------------
MODEL_PATH = r"../../models/best.pt"  # Change as per your path
...
# ---------------- EMAIL CONFIG ----------------
SENDER_EMAIL = "eduspacedevelop@gmail.com"
SENDER_PASSWORD = "vmutlxootyghdabm"
...
# ---------------- DASHBOARD TABS ----------------
tab1, tab2, tab3,tab6, tab4, tab5 = st.tabs(["📹 Monitoring", "📊 Compliance Stats", "📜 Violation Logs","📸 Violation Reports","📅 Reports & Data Export","⚙️ Settings"])

# --- TAB 1: Monitoring ---
# --- TAB 1: Monitoring ---
# --- TAB: Violation Reports ---
with tab6:
    st.markdown("## 📸 Violation Reports")
    st.info("Filter and view stored violation snapshots by date range, guard ID, and violation type.")
    ...
    with col1:
        start_date = st.date_input(
            ...
            key="report_start_date_tab6"
        )
    with col2:
        end_date = st.date_input(
            "End Date",
            value=datetime.now().date(),
            key="report_end_date_tab6"
        )

    # --- Guard filter (optional) ---
    guard_id = st.text_input("Filter by Guard ID (optional)", key="guard_filter_tab6")

    # --- Violation type filter ---
    violation_options = ["NO-MASK", "NO-HELMET", "PHONE-USAGE", "MULTIPLE PERSONS", "OTHER"]
    selected_violations = st.multiselect(
        "Select Violation Type(s)",
        options=violation_options,
        default=[],
        help="You can select multiple types (e.g., NO-MASK + PHONE-USAGE).",
        key="violation_type_tab6"
    )
    ...
    if os.path.exists(LOG_FILE):
        ...
        if not df.empty:
            ...
            if guard_id:
                mask &= df['PersonID'].astype(str).str.contains(guard_id, case=False, na=False)

            if selected_violations:
                # Match any of the selected violations (including combined ones)
                mask &= df['ViolationType'].apply(
                    lambda x: any(v in str(x) for v in selected_violations)
                )

            filtered_df = df[mask]

            if not filtered_df.empty:
                st.success(f"✅ Found {len(filtered_df)} violations between {start_date} and {end_date}")
                ...
                for idx, (_, row) in enumerate(filtered_df.iterrows()):
                    ...
                    with col:
                        ...
                        if os.path.exists(str(row['ScreenshotPath'])):
                            st.image(row['ScreenshotPath'], caption=row['ViolationType'], width=250)
                        else:
                            st.warning("⚠️ Screenshot missing.")

                # --- Download filtered data ---
                csv_data = filtered_df.to_csv(index=False)
                b64 = base64.b64encode(csv_data.encode()).decode()
                st.markdown(
                    f'<a href="data:file/csv;base64,{b64}" download="Filtered_Violations.csv">📥 Download Filtered CSV</a>',
                    unsafe_allow_html=True
                )
            else:
                st.info("ℹ️ No violations found for the selected filters.")
        else:
            st.warning("⚠️ Violation log is empty.")
    else:
        st.error("❌ Violation log file not found.")
...
🗓️ Date Range: {start_date} → {end_date}
...
"""
            ...
            st.warning("⚠️ No violations found for the selected filters.")
    else:
...
with tab4:
    ...
    if df.empty:
        st.info("⚠️ No data available to generate reports yet.")
    else:
        ...
        if filtered_df.empty:
            ...
            if st.button("📥 Generate Report ZIP", key="tab4_generate_zip"):
                try:
                    range_label = f"{start_date}_to_{end_date}".replace(":", "-")
...
with tab5:
    ...
    if st.button("💾 Save Report Preferences"):
        st.success(f"✅ Preferences saved! Default mode: {default_report_mode}, CSV name: {default_csv_name}")
    st.write("")  # Adds a blank line
    # ---------------- Auto Email Sender ----------------
    st.markdown("### ⏰ Automatic Report Emails")
    st.info("Enable this option to automatically send the latest violation report every 1 minute.")

    enable_auto_email = st.checkbox(
        "📤 Enable Auto Email Reports (every 1 minute)",
        value=st.session_state.auto_email_enabled
    )

    def send_violation_csv():
        global auto_email_flag
        while auto_email_flag:
            try:
                if os.path.exists(LOG_FILE):
                    df = pd.read_csv(LOG_FILE)
                    if not df.empty:
                        # Prepare email
                        msg = EmailMessage()
                        msg["Subject"] = f"🕒 SafetyEye Violation Report - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                        msg["From"] = SENDER_EMAIL
                        msg["To"] = SENDER_EMAIL
                        msg.set_content("Attached is the latest violation report generated automatically.")
                    # Attach CSV
                        csv_data = df.to_csv(index=False)
                        msg.add_attachment(csv_data, subtype="csv", filename="violations_auto.csv")
                    # Send email
                        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as smtp:
                            smtp.login(SENDER_EMAIL, SENDER_PASSWORD)
                            smtp.send_message(msg)
                        print(f"[AUTO EMAIL] Report sent at {datetime.now()}")
                else:
                    print("[AUTO EMAIL] No log file found.")
            except Exception as e:
                print(f"[AUTO EMAIL ERROR]: {e}")

            time.sleep(24*60*60)  # wait 5 minutes

# Handle checkbox logic
    if enable_auto_email:
        if not auto_email_flag:
            auto_email_flag = True
            threading.Thread(target=send_violation_csv, daemon=True).start()
            st.success("✅ Auto email enabled — reports will be sent every 5 minutes.")
    else:
        auto_email_flag = False
